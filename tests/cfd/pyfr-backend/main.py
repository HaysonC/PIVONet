...
def run_pyfr_simulation(backend="metal"):
    ...
    subprocess.run(["pyfr", "import", MESH_FILE, PYFRM_FILE], cwd=SIM_DIR, check=True)

    print("Starting PyFR simulation...")

    subprocess.run(
        ["pyfr", "run", "-b", backend, PYFRM_FILE, CFG_FILE], cwd=SIM_DIR, check=True
    )
...
def vtu_velocity_to_video(
    vtu_files,
    video_path="velocity.mp4",
    ...
    for f in vtu_files:
        ...
        if vel.shape[0] != pts.shape[0]:
            raise ValueError(
                f"Number of vectors ({vel.shape[0]}) does not match number of points ({pts.shape[0]})"
            )
...
def main():
    ...
    vtu_velocity_to_video(
        vtu_files,
        video_path="velocity_field.mp4",
        vector_field="Velocity",
        fps=10,
        scale=0.1,
    )
