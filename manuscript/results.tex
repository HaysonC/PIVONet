...
\subsection{Generalization Across Flow Regimes}
Generalization is evaluated using the viscous shock tube scenario, which is characterized by sharp gradients and significant dissipative effects. As shown in Fig.~10, the CNF–VSDE model maintains close agreement with ground-truth trajectories, while the CNF-only baseline exhibits larger deviations. The corresponding MAE distribution in Fig.~12 demonstrates a consistent shift toward lower error for the VSDE model across the trajectory ensemble.

These results indicate that the proposed framework generalizes effectively to shock-dominated viscous flows without requiring architectural modification, thereby supporting the hypothesis.
