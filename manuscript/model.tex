...
By adding the varieational component, our model tend to explore different pathways when reconstructing the same input; This results in a higher stablity as the model is more likely to stay on the learned manifold, as the varieatioanl component can help nudge the trajectory back when it starts to drift away.
...
\end{equation}
where \(x=(z, ctx, \text{tizme emb})\), \(\sigma_i\) are nonlinearities activation functions, and \(\{W_i, b_i\}\) are learned weights and biases for layer \(i\).
...
\end{equation}
which lets gradients flow through the encoder parameters \((\mu, \sigma)\) when we backpropagate the loss. The gradient of the ELBO with respect to the parameters \(\theta\) is
\begin{align}
