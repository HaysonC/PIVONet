...
\begin{equation}
    ...
    \label{eq:langevin}
\end{equation}
This is a SDE where $\mathbf{u}(\mathbf{x}_t)$\footnote{The simulation to obtain $u$ will be discussed in Section \ref{sec:xsimulation}} is the local fluid velocity field (Fig.~\ref{fig:cfd_langevin}a), $D$ is the diffusion coefficient, and $\mathbf{W}_t$ is a two-dimensional Brownian motion, neglecting inertial effects. 
...
\begin{figure}[h!]
    ...
    $dx_t = u(x_t)dt + \sqrt{2D}dW_t$ governs particle motion in fluid flows.
(a) Poiseuille velocity profile.
(b) Single particle trajectory.
(c) Multiple stochastic realizations from the same initial condition.
}
