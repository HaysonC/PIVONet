...
def analyze_drift(vsde_checkpoint: Path, bundle_path: Path, device: str = "cpu"):
    ...
    print(f"Initial positions range: x=[{z0[:, 0].min():.3f}, {z0[:, 0].max():.3f}], "
    ...
    with torch.no_grad():
        ...
        for t_val in [0.0, 0.5, 1.0]:
            ...
            # Statistics
            print(f"\n--- t = {t_val:.1f} ---")
            print(f"CNF drift:       x_mean={cnf_drift[:, 0].mean():.6f}, x_std={cnf_drift[:, 0].std():.6f}, "
                  f"y_mean={cnf_drift[:, 1].mean():.6f}, y_std={cnf_drift[:, 1].std():.6f}")
            print(f"                 magnitude_mean={cnf_drift.norm(dim=1).mean():.6f}")
            print(f"Posterior drift: x_mean={posterior_drift[:, 0].mean():.6f}, x_std={posterior_drift[:, 0].std():.6f}, "
                  f"y_mean={posterior_drift[:, 1].mean():.6f}, y_std={posterior_drift[:, 1].std():.6f}")
            print(f"                 magnitude_mean={posterior_drift.norm(dim=1).mean():.6f}")
            
            # Check if y-component is non-trivial
            y_ratio = posterior_drift[:, 1].abs().mean() / (posterior_drift[:, 0].abs().mean() + 1e-12)
            print(f"Y/X ratio: {y_ratio:.4f} (>0.1 indicates significant 2D learning)")


def main():
    if len(sys.argv) < 3:
        print("Usage: python scripts/analyze_drift_components.py <vsde_checkpoint> <bundle.npz>")
        sys.exit(1)
