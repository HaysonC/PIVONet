...
class ODEFunc(nn.Module):
    def __init__(
        ...
        self.net = MLP(
            dim + cond_dim + 1, dim, hidden_dim, depth=depth, dropout_p=dropout_p
        )
    ...
    def forward(
        ...
        assert self._context is not None, (
            "Context not set in ODEFunc. Call set_context before integration."
        )
        B = z.size(0)
...
class CNF(nn.Module):
    def eval_field(
        self, z: torch.Tensor, context: torch.Tensor, t: float = 1.0
    ) -> torch.Tensor:
        ...
        dtype = (
            z.dtype
            if z.dtype in (torch.float32, torch.float16, torch.bfloat16)
            else torch.float32
        )
        ...
        try:
            f, _ = self.func(
                torch.tensor(float(t), device=device, dtype=dtype), (z, logp0)
            )
        finally:
    ...
    def _integrate_fixed(
        self, x: torch.Tensor, logp: torch.Tensor, tspan: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(tspan.numel() - 1):
            ...
            k2_z, k2_lp = self.func(
                ti + 0.5 * h, (z + 0.5 * h * k1_z, lp + 0.5 * h * k1_lp)
            )
            k3_z, k3_lp = self.func(
                ti + 0.5 * h, (z + 0.5 * h * k2_z, lp + 0.5 * h * k2_lp)
            )
    ...
    def flow(
        ...
        if stochastic_steps_jitter and self.training:
            s = int(
                max(
                    ...
                    steps
                    + torch.randint(
                        low=-stochastic_steps_jitter,
                        high=stochastic_steps_jitter + 1,
                        size=(1,),
                    ).item(),
                )
            )
        ...
        if x.device.type == "mps":
            ...
            res = odeint_adj(
                self.func, (x, logp0), tspan, atol=atol, rtol=rtol, method=solver
            )
    ...
    def log_prob(
        ...
        z, delta_logp = self.flow(
            x,
            context,
            t0=1.0,
            t1=0.0,
            steps=steps,
            stochastic_steps_jitter=stochastic_steps_jitter,
            solver=solver,
            rtol=rtol,
            atol=atol,
        )
        base_logprob = (
            -0.5 * ((z / base_std) ** 2).sum(dim=1, keepdim=True)
            - 0.5
            * self.dim
            * torch.log(torch.tensor(2 * torch.pi, device=x.device, dtype=x.dtype))
            - self.dim
            * torch.log(torch.tensor(base_std, device=x.device, dtype=x.dtype))
        )
    ...
    def sample(
        ...
        dtype = (
            context.dtype
            if context.dtype in (torch.float32, torch.float16, torch.bfloat16)
            else torch.float32
        )
        context = context.to(device=device, dtype=dtype)
        ...
        if context.device.type == "mps":
            ...
            z_traj, _ = cast(
                tuple[torch.Tensor, torch.Tensor],
                odeint_adj(self.func, (z0, logp0), tspan, atol=1e-5, rtol=1e-5),
            )
        x = z_traj[-1]
    ...
    def rollout_from(
        ...
        dtype = (
            x0.dtype
            if x0.dtype in (torch.float32, torch.float16, torch.bfloat16)
            else torch.float32
        )
        ...
        tspan = torch.linspace(
            0.0, 1.0, steps=max(2, int(steps)), device=device, dtype=dtype
        )
        ...
        if device.type == "mps":
            ...
            z_traj, _ = cast(
                tuple[torch.Tensor, torch.Tensor],
                odeint_adj(self.func, (x0, logp0), tspan, atol=1e-5, rtol=1e-5),
            )
        out = z_traj.permute(1, 0, 2).contiguous()
        if enforce_bounds and H is not None:
            y = out[:, :, 1]
