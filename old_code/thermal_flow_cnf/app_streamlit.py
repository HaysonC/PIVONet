...
from thermal_flow_cnf.src.flows import (
    poiseuille_flow,
    diffuser_flow,
    compressor_flow,
    bend_flow,
...
from thermal_flow_cnf.src.simulation.dataset import (
    TrajectoryDataset,
    TrajectorySequenceDataset,
)
from thermal_flow_cnf.src.evaluation.visualize import (
    plot_trajectories,
    plot_density_hist2d,
    animate_trajectories,
    animation_to_html,
)
from thermal_flow_cnf.src.evaluation.metrics import (
...
with st.sidebar:
    ...
    flow = st.selectbox(
        "Flow",
        ["poiseuille", "diffuser", "compressor", "bend"],
        index=0,
        key="flow_select",
    )
    ...
    L = st.number_input(
        "Length L (diffuser/compressor/bend)", value=1.0, step=0.1, format="%.2f"
    )
    ...
    if prev_flow != flow:
        if flow == "diffuser":
            ...
            st.session_state["H_out"] = max(
                st.session_state.get("H_out", H), st.session_state.get("H_in", H) * 1.5
            )
            ...
            st.session_state["H_out"] = max(
                0.05,
                min(
                    st.session_state.get("H_out", H),
                    st.session_state.get("H_in", H) * 0.7,
                ),
            )
        st.session_state["_prev_flow"] = flow
    H_in = st.number_input(
        "H_in (diffuser/compressor)",
        value=float(st.session_state["H_in"]),
        step=0.1,
        format="%.2f",
        key="H_in",
    )
    H_out = st.number_input(
        "H_out (diffuser/compressor)",
        value=float(st.session_state["H_out"]),
        step=0.1,
        format="%.2f",
        key="H_out",
    )
    ...
    if flow == "diffuser":
        if H_out <= H_in:
            ...
            st.warning(
                "For a diffuser, H_out should be greater than H_in. Adjusted H_out automatically."
            )
            adjusted = True
    elif flow == "compressor":
        if H_out >= H_in:
            ...
            H_out = float(new_H_out)
            st.warning(
                "For a compressor, H_out should be less than H_in. Adjusted H_out automatically."
            )
            adjusted = True
    Umax_in = st.number_input(
        "Umax_in (diffuser/compressor)", value=CONFIG["Umax"], step=0.1
    )
    bend_angle = st.number_input(
        "Bend angle (deg)", value=90.0, step=5.0, format="%.1f"
    )
    ...
    dropout_p = st.slider(
        "Dropout p (stochastic)",
        min_value=0.0,
        max_value=0.5,
        value=0.0,
        step=0.05,
        help="Introduces stochasticity for training/inference when model in train mode",
    )
    ...
    selected_device = st.selectbox(
        ...
        dev_opts,
        index=default_idx,
        help="Select hardware device for model training/inference",
    )
    ...
    overlay_mode = st.selectbox(
        ...
        ["quiver", "stream"],
        index=0,
        help="Show vector field as quiver or streamlines",
    )
...
with tabs[0]:
    ...
    with colA:
        init_dist = st.selectbox(
            "Initial distribution", ["uniform", "gaussian"], index=1
        )
    with colB:
        mu_x = st.number_input("mu_x", value=0.0, step=0.1)
        mu_y = st.number_input("mu_y", value=0.0, step=0.1)
    with colC:
        cov_xx = st.number_input(
            "var_x", value=0.1, step=0.05, min_value=0.0, format="%.3f"
        )
        cov_yy = st.number_input(
            "var_y", value=0.1, step=0.05, min_value=0.0, format="%.3f"
        )
    ...
    if st.button("Simulate", key="simulate_btn"):
        ...
        log(
            f"[simulate] flow={flow}, N={int(num_particles)}, T={int(T)}, dt={float(dt):.3g}, D={float(D):.3g}, H={float(H):.3g}"
        )
    ...
    if dataset_path:
        ...
        st.write(
            f"MSD ({msd_mode}): {mean_squared_displacement(trajs, mode=msd_mode):.4f}"
        )
...
with tabs[1]:
    ...
    data_noise_std = st.number_input(
        "Target jitter std (stochastic)",
        min_value=0.0,
        max_value=0.5,
        value=0.0,
        step=0.01,
    )
    steps_jitter = st.number_input(
        "ODE steps jitter (+/-)",
        min_value=0,
        max_value=8,
        value=0,
        step=1,
        help="Randomize ODE steps per batch to regularize",
    )
    ...
    no_slip_coef = st.number_input(
        "No-slip loss coeff", min_value=0.0, max_value=10.0, value=1.0, step=0.1
    )
    enable_bern = st.checkbox(
        "Enable Bernoulli/Poiseuille shape loss",
        value=False,
        help="Encourage consistent kinetic energy / parabolic profile",
    )
    bern_coef = st.number_input(
        "Bernoulli loss coeff", min_value=0.0, max_value=10.0, value=0.0, step=0.1
    )
    if dataset_path and st.button("Train CNF", key="train_btn"):
        ...
        log(
            f"[train] device={device}, hidden_dim={int(hidden_dim)}, dropout={float(dropout_p):.2f}, epochs={int(epochs)}, batch={int(batch)}, noise_std={float(data_noise_std):.3f}, steps_jitter=+/-{int(steps_jitter)}"
        )
        model = CNF(
            dim=2,
            cond_dim=3,
        ...
        def tcb(epoch, epochs, step, total, loss):
            ...
            p_outer.progress(
                ...
                text=f"Training epoch {epoch}/{epochs} | NLL {loss:.4f} | avg logp {avg_logp:.4f} | bpd {bpd:.3f}",
            )
            ...
            if total > 0 and (step == total or step % max(1, total // 10) == 0):
                log(
                    f"[train] epoch {epoch}/{epochs} step {step}/{total} NLL={loss:.4f} avg_logp={avg_logp:.4f} bpd={bpd:.3f}"
                )
            if step == total:
    ...
    if ckpts:
        default_ckpt_idx = (
            ckpts.index(current_ckpt) if current_ckpt in ckpts else len(ckpts) - 1
        )
        sel = st.selectbox(
            "Select Checkpoint (optional)",
            ["<none>"] + ckpts,
            index=(default_ckpt_idx + 1),
        )
        st.session_state["model_ckpt"] = None if sel == "<none>" else sel
        st.caption(
            f"Selected: {sel if sel != '<none>' else 'no checkpoint (use fresh model)'}"
        )
    else:
    ...
    with vsde_cols[0]:
        vsde_epochs = st.number_input(
            "SDE epochs", min_value=1, max_value=200, value=5, step=1
        )
    with vsde_cols[1]:
        vsde_lr = st.number_input("SDE learning rate", value=5e-4, format="%.1e")
    with vsde_cols[2]:
        vsde_batch = st.number_input(
            "SDE batch size", min_value=4, max_value=512, value=int(batch), step=4
        )
    vsde_cols2 = st.columns(3)
    with vsde_cols2[0]:
        vsde_particles = st.number_input(
            "Particles", min_value=1, max_value=16, value=4, step=1
        )
    with vsde_cols2[1]:
        vsde_obs_std = st.number_input(
            "Observation std",
            min_value=0.001,
            max_value=0.5,
            value=0.05,
    ...
    with vsde_cols2[2]:
        vsde_steps = st.number_input(
            "SDE steps", min_value=10, max_value=400, value=120, step=10
        )
    vsde_cols3 = st.columns(3)
    with vsde_cols3[0]:
        vsde_kl = st.number_input(
            "KL warmup", min_value=0.0, max_value=5.0, value=1.0, step=0.1
        )
    with vsde_cols3[1]:
        vsde_ctrl = st.number_input(
            "Control cost scale", min_value=0.0, max_value=10.0, value=1.0, step=0.1
        )
    with vsde_cols3[2]:
        default_ctx_dim = int(st.session_state.get("vsde_ctx_dim", 128))
        vsde_ctx_dim = st.number_input(
            "Posterior ctx dim",
            min_value=32,
            max_value=512,
            value=default_ctx_dim,
            step=32,
        )
    ...
    if (
        dataset_path
        and ckpt_path
        and st.button("Train Variational SDE", key="train_vsde_btn")
    ):
        ...
        def vsde_cb(epoch, epochs, step, total, loss, stats=None):
            ...
            vsde_inner.progress(
                min(step / total, 1.0),
                text=f"Batch {step}/{total} | control {ctrl if ctrl is not None else float('nan'):.4f}",
            )
            if total > 0 and (step == total or step % max(1, total // 8) == 0):
                log(
                    f"[vsde-train] epoch {epoch}/{epochs} step {step}/{total} loss={loss:.4f} stats={stats}"
                )
            if step == total:
        ...
        st.info(
            "Select a CNF checkpoint above to condition the variational SDE trainer."
        )
...
with tabs[2]:
    ...
    with cA:
        max_frames = st.number_input(
            ...
            value=300,
            step=50,
            help="Cap frames to keep embed small",
        )
    with cB:
        frame_stride_in = st.number_input(
            "Frame stride (0=auto)", min_value=0, max_value=50, value=0, step=1
        )
    with cC:
        embed_limit_mb = st.number_input(
            "Embed limit (MB)", min_value=5.0, max_value=200.0, value=20.0, step=1.0
        )
        # Finer inference controls
        infer_steps = st.number_input(
            "Integration steps (fine)",
            min_value=10,
            max_value=500,
            value=150,
            step=10,
            help="ODE integration steps for model rollout (higher = smoother trajectory)",
        )
        infer_solver = st.selectbox("Solver", ["dopri5", "rk4"], index=0)
        infer_rtol = st.number_input(
            "rtol", min_value=1e-7, max_value=1e-2, value=1e-5, step=1e-6, format="%.1e"
        )
        infer_atol = st.number_input(
            "atol", min_value=1e-7, max_value=1e-2, value=1e-5, step=1e-6, format="%.1e"
        )
    ...
    if vsde_ckpts:
        default_vsde = (
            vsde_ckpts.index(current_vsde)
            if current_vsde in vsde_ckpts
            else len(vsde_ckpts) - 1
        )
        sel_vsde = st.selectbox(
            "Select Variational SDE checkpoint",
            ["<none>"] + vsde_ckpts,
            index=default_vsde + 1,
        )
        st.session_state["vsde_ckpt"] = None if sel_vsde == "<none>" else sel_vsde
    else:
        st.caption(
            "No variational SDE checkpoints yet — train one in the Train tab to enable stochastic inference."
        )

    vsde_ckpt_path = st.session_state.get("vsde_ckpt")
    vsde_eval_cols = st.columns(3)
    with vsde_eval_cols[0]:
        vsde_eval_particles = st.number_input(
            "Posterior particles (eval)", min_value=1, max_value=32, value=4, step=1
        )
    with vsde_eval_cols[1]:
        vsde_eval_steps = st.number_input(
            "SDE steps (eval)", min_value=10, max_value=500, value=150, step=10
        )
    with vsde_eval_cols[2]:
        vsde_eval_obs = st.number_input(
            "Obs std (eval)",
            min_value=0.001,
            max_value=0.5,
            value=0.05,
    ...
    vsde_eval_batch = st.number_input(
        "Eval batch size", min_value=8, max_value=128, value=32, step=8
    )
    vsde_eval_anchor_noise = st.number_input(
        "Anchor noise",
        min_value=0.0,
        max_value=0.5,
        value=0.02,
    ...
    if dataset_path and st.button("Animate", key="animate_btn"):
        ...
        context = torch.from_numpy(np.concatenate([x0s, thetas], axis=1)).to(
            torch.float32
        )
        device = selected_device
        ...
        if ckpt_path and os.path.isfile(ckpt_path):
            try:
                ...
                st.warning(
                    f"Checkpoint incompatible or unreadable, running without loading: {e}"
                )
        ...
        log(
            f"[animate] device={device}, n_true={trajs.shape[0]}, T_true={trajs.shape[1]}, n_pred_req={n_samp}"
        )
        with torch.no_grad():
            ...
            x0_batch = torch.from_numpy(x0s[:n_samp]).to(
                device=device, dtype=torch.float32
            )
            z_pred = model.rollout_from(
                x0_batch, ctx_batch, steps=steps_anim, H=float(H), enforce_bounds=True
            )
        ...
        try:
            anim = animate_trajectories(
                ...
                frame_stride=(
                    int(frame_stride_in) if int(frame_stride_in) > 0 else None
                ),
            )
            components.html(
                animation_to_html(anim, embed_limit_mb=float(embed_limit_mb)),
                height=420,
            )
    ...
    if (
        ...
        and vsde_ckpt_path
        and ckpt_path
        and st.button("Sample Variational Posterior", key="sample_vsde_btn")
    ):
        device = selected_device
        data = np.load(dataset_path)
        init_mean_loaded = data.get("init_mean")
        ...
        traj_samples_np = (
            traj_samples.permute(1, 2, 0, 3).contiguous().cpu().numpy()
        )  # (P, B, T, D)
        ...
        trajs_pred = trajs_pred.reshape(
            p_demo * n_demo, trajs_pred.shape[2], trajs_pred.shape[3]
        )
        ...
        components.html(
            animation_to_html(anim_vsde, embed_limit_mb=float(embed_limit_mb)),
            height=420,
        )
        ...
        st.warning(
            "Select a CNF checkpoint to pair with the variational posterior sampler."
        )
...
with tabs[3]:
    ...
    st.caption(
        "Interactive 3D animation of particle trajectories (in-app, no external window)."
    )
    # Select source: choose dataset or use current CSV
    ds_choice = st.selectbox(
        "Source dataset (prefer data/<name>/ with positions_frame_*.bin)",
        options=["current_sim_csv"]
        + sorted([p.name for p in data_dir.iterdir() if p.is_dir()]),
        index=0,
    )
    max_points = st.number_input(
        "Max particles to display", min_value=100, max_value=50000, value=2000, step=100
    )
    frame_step = st.number_input(
        "Frame stride", min_value=1, max_value=50, value=1, step=1
    )
        ...
        if ds_choice == "current_sim_csv":
            ...
            if not csv_path.exists():
                ...
                sel_pids = (
                    pids
                    if N <= max_points
                    else np.random.choice(pids, size=max_points, replace=False)
                )
                frames = []
                for t in sorted(tids)[::frame_step]:
                    sub = df[df["time_step"] == t]
                    coords = (
                        sub.set_index("particle_id")
                        .loc[sel_pids][["x", "y"]]
                        .to_numpy()
                    )
...
with tabs[4]:
    ...
    if dataset_path:
        ...
        if do_compute:
            ...
            thetas = np.zeros((x0s.shape[0], 1), dtype=np.float32)
            context = (
                torch.from_numpy(np.concatenate([x0s, thetas], axis=1))
                .to(torch.float32)
                .to(selected_device)
            )
            model = CNF(
            ...
            with torch.no_grad():
                ...
                pred = model.sample(
                    n=n_eval,
                    context=context[:n_eval],
                    steps=int(min(64, trajs.shape[1])),
                    H=float(H),
                    enforce_bounds=True,
                )
