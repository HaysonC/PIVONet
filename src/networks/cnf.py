        ...
        with torch.enable_grad():
            ...
            if self.training and not self.skip_divergence:
                ...
                grad = torch.autograd.grad(
                    f_eps, z, create_graph=True, retain_graph=True, allow_unused=True
                )[0]
                if grad is None:
...
class CNFModel(nn.Module):
    ...
    **Key Design Decisions**:
    ...
    def _integrate(
        self, x: torch.Tensor, context: torch.Tensor, tspan: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        **Validation Logic** (lines 84-95):
        ...
        if tspan.numel() < 2:
            raise ValueError(
                "tspan must contain at least two time points for integration"
            )
        ...
        res = odeint(
            self.func, (x, logp0), tspan, atol=1e-5, rtol=1e-5, options=solver_options
        )
        assert res is not None, "ODE integration failed during flow"
        z_traj, logp_traj = res
        assert z_traj is not None and logp_traj is not None, (
            "ODE integration returned None states"
        )
    ...
    def log_prob(
        self, x: torch.Tensor, context: torch.Tensor, base_std: float = 1.0
    ) -> torch.Tensor:
        """
        ...
        **Usage Example**::
        ...
        normalizer = torch.log(
            torch.tensor(2 * torch.pi * base_std**2, device=device, dtype=z.dtype)
        )
        base = (
            -0.5 * ((z / base_std) ** 2).sum(dim=1, keepdim=True)
            - 0.5 * self.dim * normalizer
        )
    ...
    @torch.no_grad()
    def sample(
        self, n: int, context: torch.Tensor, base_std: float = 1.0, steps: int = 30
    ) -> torch.Tensor:
        """
        ...
        **Usage Example**::
        ...
        res = odeint(
            self.func,
            (z0, torch.zeros(n, 1, device=device, dtype=dtype)),
            tspan,
            atol=1e-5,
            rtol=1e-5,
