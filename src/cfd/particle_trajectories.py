...
class ParticleTrajectorySimulator:
    ...
    **Key Design Decisions**:
        ...
        **Usage Example**::
        ...
        for step_index, (snapshot_time, field) in enumerate(
            ...
            if not np.isfinite(diffusion_noise).all():
                ...
                diffusion_noise = np.where(
                    ~np.isfinite(diffusion_noise), 0.0, diffusion_noise
                )
            ...
            if self.respect_snapshot_timing:
                ...
                previous_time = (
                    previous_time + dt_step if previous_time is not None else dt_step
                )
    ...
    def _derive_timestep(
        self, previous_time: float | None, snapshot_time: float
    ) -> float:
        """
    ...
    def _sample_velocity_at_positions(
        self, field: np.ndarray, positions: np.ndarray
    ) -> np.ndarray:
        """
