...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of particles to simulate. Falls back to config value if omitted.",
    )
    parser.add_argument(
        "--max-steps",
        type=int,
        default=None,
        help="Maximum number of velocity snapshots to consume. Falls back to config value if omitted.",
    )
    parser.add_argument(
        "--dt",
        type=float,
        default=None,
        help="Fallback timestep when snapshot timing is not respected. Falls back to config value if omitted.",
    )
    parser.add_argument(
        "--output-dir",
        default="data/double-mach/trajectories",
        help="Directory to write trajectory bundles into.",
    )
    parser.add_argument(
        "--velocity-dir",
        default=None,
        help="Optional path to the directory containing velocity .npy snapshots (overrides config).",
    )
    parser.add_argument(
        "--mesh-path",
        default=None,
        help="Optional explicit path to mesh point coordinates (.npy). Defaults to autodiscovery near the velocity directory.",
    )
    parser.add_argument(
        "--system",
        default=None,
        help="Name/slug of the CFD system to store inside the bundle metadata (defaults to inferred directory name).",
    )
    parser.add_argument(
...
def _discover_mesh_points(
    explicit: Path | None, velocity_dir: Path
) -> np.ndarray | None:
...
def _infer_system_slug(velocity_dir: Path) -> str:
    for candidate in (
        velocity_dir.name,
        velocity_dir.parent.name if velocity_dir.parent != velocity_dir else "",
    ):
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    particles = (
        ...
        else int(config.trajectory_particles)
    )
    max_steps = (
        int(args.max_steps)
        if args.max_steps is not None
        else (
            int(config.trajectory_steps)
            if config.trajectory_steps is not None
            else None
        )
    )
    ...
    velocity_dir = (
        velocity_override if velocity_override is not None else config.velocity_dir
    )
    ...
    if mesh_points is None:
        print(
            "Warning: mesh points could not be resolved; bundles will omit mesh metadata."
        )
