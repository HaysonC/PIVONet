...
def main():
    ...
    if first.suffix == '.npz':
        ...
        N = frames_np.shape[1]
        stream = False
        print(f"Loaded trajectory bundle with {num_frames} timesteps, {N} particles")
    else:
        N = first_arr.shape[0]
    ...
    while window.running:
        ...
        if lmb_pressed and not last_lmb_pressed and cnf_model is not None:
            mx, my = window.get_cursor_pos()
            # Simple mapping: assume screen 0-1 maps to world -5 to 5
            world_x = (mx - 0.5) * 10.0
            world_y = (my - 0.5) * 10.0
    ...
    # optional grid helper (simple XY grid at z=0)
        ...
        # Draw generated orange particles
        if generated_frames is not None and N_gen > 0:
